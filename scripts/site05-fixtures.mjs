/**
 * One signed-in fixture member for spec SITE-05's browser lane.
 *
 * Modelled on `scripts/site04-fixtures.mjs`, and COPIED rather than called, for
 ...
 * `site05-ui-` account cannot come out of it and a teardown run from it would
 * either miss this spec's rows or delete another spec's. Four specs have now
 * paid this; the shared thing is the shape, not the file.
 *
 * `member_allowlist` now holds the real cohort (42 rows, 22 accounts, seeded
 * 2026-09-03), which is what closed program finding 3. That makes the named-
 * address rule below matter MORE than it did when the table was empty: this
 * harness inserts exactly its own address and deletes exactly that address
 * again, never a pattern over the table, because everything else in it is a
 * real participant who would lose their account.
 *
...
const PASSWORD = 'site05-ui-fixture-passphrase'
...
async function setup() {
  ...
  const values = Object.keys(ROLES)
    .map((r) => `(${q(ADDR(r))}, ${q('SITE-05 UI fixture; delete with --teardown')})`)
    .join(', ')
...
if (mode === '--setup') await setup()
  ...
  console.error('usage: node scripts/site05-fixtures.mjs --setup | --print-ids | --teardown')
  process.exit(2)
}

...
TABLE_ORDER = ["assessment_bundle", "bundle_unit", "bundle_qualification"]
...
# Bundle-scoped grant names. A closed set, because `scope_kind = 'bundle'` is the
# one scope whose key is not a foreign key into the registry, so a typo there
# would otherwise seed a grant nobody holds and refuse every assignment.
BUNDLE_SCOPE_KEYS = {"obt-cdt-facilitator"}
